...
const registrarPropiedad = async () => {
  try {
    const response = await axios.post('http://api-domusaurea.geekcorplab.com/api/propiedades', form.value)
...
const guardarDocumentos = async () => {
  try {
    for (const item of archivos.value) {
      ...
      await axios.post('http://api-domusaurea.geekcorplab.com/api/documentos', formData, {
        headers: {
