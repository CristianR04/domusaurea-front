...
const buscar = async () => {
  try {
    const res = await axios.get(`http://api-domusaurea.geekcorplab.com/api/reportes?id_propiedad=${id_propiedad.value}`)
...
const descargarPDF = (id_propiedad) => {
  const url = `http://api-domusaurea.geekcorplab.com/api/reportes/descargar/${id_propiedad}`
